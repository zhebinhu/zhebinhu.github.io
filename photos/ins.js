        ...
        var render = function render(res) {
            ...
            for (var j = 0, len2 = res.list.length; j < len2; j++) {
                ...
                for (var i = 0, len = data.link.length; i < len; i++) {
                    var minSrc = '\n' +
                        'https://huzb.oss-cn-beijing.aliyuncs.com/min_' + data.link[i];
                    var src = 'https://huzb.oss-cn-beijing.aliyuncs.com/src_' + data.link[i];
                    var type = data.type[i];
                    var target = src/* + (type === 'video' ? '.mp4' : '.jpg')*/;
                    /*src += '.jpg';*/

                    liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">\
                <a href="' + src + '" itemprop="contentUrl" data-size="' + data.width[i] + 'x' + data.height[i] + '" data-type="' + type + '" data-target="' + target + '">\
            ...
            var openPhotoSwipe = function openPhotoSwipe(index, galleryElement, disableAnimation, fromURL) {
                ...
                options = {
                    ...
                    getThumbBoundsFn: function getThumbBoundsFn(index) {
                        ...
                        return {
                            x: rect.left - ((rect.height * (items[index].w / items[index].h )) - rect.width) / 2,
